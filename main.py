...
def start(server: str):
    ...
    if map and map in MAPS:
        subprocess.call(["echo", f"MAP={map}", "cs2-server", f"@{server}", "start"])
        flash(f"Started server {server} with map {map}")
    elif server in SERVERS:
        subprocess.call(["echo", "cs2-server", f"@{server}", "start"])
...
def stop(server: str):
    if server in SERVERS:
        subprocess.call(["echo", "cs2-server", f"@{server}", "stop"])
...
def restart(server: str):
    if server in SERVERS:
        subprocess.call(["echo", "cs2-server", f"@{server}", "restart"])
